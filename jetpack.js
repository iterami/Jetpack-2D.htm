function d(){
    if(game_running){
        ...
        if(settings[4]>0 && frames_per_obstacle>1 && frames%settings[4]===0){/*obstacle frequency increase*/
    ...
    if(i>=0){
        ...
        do{
            ...
            if(obstacles[i][0]<0){
                ...
                if(obstacles[i][0]>=x-25-obstacles[i][2]/2 && obstacles[i][0]<=x+25+obstacles[i][2]/2 && obstacles[i][1]>=j-25-obstacles[i][3]/2 && obstacles[i][1]<=j+25+obstacles[i][3]/2){
                    game_running = 0;
                    update_best()
                }
            }
...
function save(){
    ...
    i = 5;
    do{
        j = ['ms-per-frame','gravity','jetpack-power','obstacle-frequency','obstacle-increase','audio-volume'][i];
